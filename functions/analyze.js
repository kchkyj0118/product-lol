export async function onRequestPost(context) {
    ...
    try {
      ...
      if (!accRes.ok) return new Response(JSON.stringify({ error: "플레이어를 찾을 수 없습니다. 이름과 태그를 확인하세요." }), { status: 200 });
      ...
      const blueNames = pList.filter(p => p.team === 100).map(p => p.cName).join(", ");
      const redNames = pList.filter(p => p.team === 200).map(p => p.cName).join(", ");
  
      // 5. 실전 훈수 프롬프트 (요청사항 100% 반영)
      const prompt = `너는 롤 1타 강사야. 현재 게임 상성을 분석해서 이길 전략을 짜줘.
  내 챔피언: ${myChamp}
  블루팀 조합: ${blueNames}
  레드팀 조합: ${redNames}
  
  명령:
  1. 아이템, 룬 추천은 절대 하지 마. (잘 못하니까 하지 말라고 하심)
  2. [요약]: 내 챔피언이 상대보다 약한 구간과 강해지는 정확한 레벨/타이밍, 그리고 지금 당장 해야 할 핵심 행동 3줄.
  3. [상세]: 라인전 상성(피할 놈, 노릴 놈), 정글 동선 설계, 갱킹 성공률 높은 라인 찝어주기 등 아주 구체적으로 '탑은 ~해라', '미드는 ~해라' 식으로 훈수 둬.`;
      ...
      return new Response(JSON.stringify({ error: "서버 연결에 실패했습니다. API 키 설정을 확인하세요." }), { status: 200 });
    }
  }